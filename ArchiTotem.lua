...
function ArchiTotem_CastTotem()
  ...
  CastSpellByName(ArchiTotem_TotemData[this:GetName()].name);
...
function ArchiTotem_Switch(arg1, arg2)
  ...
  -- DEFAULT_CHAT_FRAME:AddMessage(ArchiTotem_GetSpellId(ArchiTotem_TotemData[arg1].name));
...
function ArchiTotem_UpdateTextures()
  ...
  for k, v in totemElements do
    ...
    for i = 1, ArchiTotem_Options[threeLetterElement].max do
      -- For all buttons of that element
      getglobal("ArchiTotemButton_" .. v .. i .. "Texture"):SetTexture(ArchiTotem_TotemData["ArchiTotemButton_" .. v .. i].icon);
      -- Set the texture
    end
  end
end
...
function ArchiTotem_SetDirection(dir)
  ...
  for k, v in totemElements do
    ...
    for i = 2, ArchiTotem_Options[threeLetterElement].max do
      -- For all buttons of that element
      local relativeTotem = getglobal("ArchiTotemButton_" .. v ..(i - 1));
      -- The totem to be anchored to
      getglobal("ArchiTotemButton_" .. v .. i):ClearAllPoints()
      -- Clear all anchors, or the buttons will be messed up
      getglobal("ArchiTotemButton_" .. v .. i):SetPoint(anchor1, relativeTotem, anchor2);
      -- Set the anchor
    end
  end
end
...
function ArchiTotem_Order(first, second, third, forth)
  ...
  getglobal(firstButton):ClearAllPoints()
  -- Clear all anchors, or the buttons will be messed up
  getglobal(firstButton):SetPoint("CENTER", ArchiTotemFrame, "CENTER");
  -- Set the anchor

  getglobal(secondButton):ClearAllPoints()
  -- Clear all anchors, or the buttons will be messed up
  getglobal(secondButton):SetPoint("BOTTOMLEFT", firstButton, "BOTTOMRIGHT");
  -- Set the anchor

  getglobal(thirdButton):ClearAllPoints()
  -- Clear all anchors, or the buttons will be messed up
  getglobal(thirdButton):SetPoint("BOTTOMLEFT", secondButton, "BOTTOMRIGHT");
  -- Set the anchor

  getglobal(forthButton):ClearAllPoints()
  -- Clear all anchors, or the buttons will be messed up
  getglobal(forthButton):SetPoint("BOTTOMLEFT", thirdButton, "BOTTOMRIGHT");
  -- Set the anchor
end
...
function ArchiTotem_SetScale(scale)
  ...
  for k, v in totemElements do
    ...
    for i = 1, ArchiTotem_Options[threeLetterElement].max do
      -- For all buttons of that element
      getglobal("ArchiTotemButton_" .. v .. i):SetScale(scale)
      -- Set the scale
    end
  end
end
